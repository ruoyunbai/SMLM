...
with torch.no_grad():
    for x, y,z in dl:
        ...
        pred = model(x)
        print(f"photon min:{pred.min()},max:{pred.max()},media:{pred.median()}")
        ...
        for region in measure.regionprops(all_labels):
            if region.area <mean_area-0.2*std_area:
        ...
        if idx==0:
            ...
            ax[2].imshow(mask)
            ax[2].scatter(x=[p[1] for p in centroids], y=[p[0] for p in centroids], c='r', s=1)
            y=(y-y.min())/(y.max()-y.min())

            # ax[3].imshow(pred.squeeze()-pred.squeeze().mean())
            # ax[4].imshow(y.squeeze()>y.mean()+3*y.std())
